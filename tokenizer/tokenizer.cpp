...
namespace miniplc0 {
    std::optional<int32_t> parseUnsignedInt(std::stringstream& ss)
    {
        std::string ts; ss >> ts;
        int index = 0, len = ts.size();
        while (index < len - 1)
        {
            if (ts[index] == '0') index++;
            else break;
        ...
        const long long ti = std::stoll(ts);
        if (ti < 0 || ti>2147483647)
        {
            return {};
        }
    ...
    std::pair<std::optional<Token>, std::optional<CompilationError>> Tokenizer::nextToken() {
        ...
        while (true) {
            ...
            case UNSIGNED_INTEGER_STATE: {
                ...
                if (!current_char.has_value())
                {
                    auto tl = parseUnsignedInt(ss);
                    if (tl.has_value())
                    {
                        return { std::make_optional<Token>(UNSIGNED_INTEGER, tl.value(), pos, currentPos()),{} };
                    }
                ...
                if (miniplc0::isdigit(ch))
                    ...
                    ss << ch, current_state = DFAState::IDENTIFIER_STATE;
                else
                {
                    unreadLast();
                    auto tl = parseUnsignedInt(ss);
                    ...
                    return { {},std::make_optional<CompilationError>(0, 0, ErrorCode::ErrIntegerOverflow) };
                }
                break;
            }
            case IDENTIFIER_STATE: {
                ...
                {
                    std::string ts; ss >> ts;
                    if (keywords.count(ts))
                    {
                        return { std::make_optional<Token>(keywords.at(ts), ts, pos, currentPos()),{} };
                    }
                    return { std::make_optional<Token>(IDENTIFIER, ts, pos, currentPos()),{} };
                }
                auto ch = current_char.value();
            ...
            case PLUS_SIGN_STATE: {
                ...
                // 这里怪怪的，理论上来说你每一行后面铁定加了个\n，+后面不可能是eof，如果直连eof的话这里两个pos一样。其他符号的话也一样
