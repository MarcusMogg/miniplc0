...
#include <climits>
#include <3rd_party/catch2/catch2/catch.hpp>
...
namespace miniplc0 {
    ...
    std::optional<CompilationError> Analyser::analyseVariableDeclaration() {
        ...
        while (true)
        {
            ...
            if (!tmp.has_value())
                return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrNoSemicolon);
            if (tmp.value().GetType() == SEMICOLON)
            {
            ...
            else
            {
                ...
                // '='
                if (tmp.value().GetType() != EQUAL_SIGN)
    ...
    std::optional<CompilationError> Analyser::analyseConstantExpression(int32_t& out) {
        ...
        if (!next.has_value() || next.value().GetType() != UNSIGNED_INTEGER)
            return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrConstantNeedValue);
        out = out * std::any_cast<std::int32_t> (next.value().GetValue());
        return {};
    }
    ...
    std::optional<CompilationError> Analyser::analyseAssignmentStatement() {
        ...
        // 需要生成指令吗？
        auto next = nextToken();
        auto id = next.value().GetValueString();
        ...
        {
            _vars[id] = _uninitialized_vars[id];
